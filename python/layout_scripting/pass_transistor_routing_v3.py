...
def via1_array(cell, via1_layer, x0, y0, num_row, num_col, array_sep, via_size):
    for i in range(num_row):
        for j in range(num_col):
            via1(cell, via1_layer, x0+j*array_sep+j*via_size, y0+i*array_sep+i*via_size, via_size)
...
def source_connection(cell, layout, x0, y0, length, gates_num, difusion_hsp, path_width):
    ...
    for j in range(int(gates_num/2)+1):
        ...
        via1_array(cell, via2_layer, x0_0+(300-190)/2, y0+tran_w+path_width+220+h_path_sep_width+(300-190)/2, 66, 1, 220, 190)
...
def drain_connection(cell, layout, x0, y0, length, gates_num, difusion_hsp, path_width):
    ...
    for j in range(int(gates_num/2)+1):
        ...
        cell.shapes(layer).insert(m1_path)
        cell.shapes(m2_layer).insert(src_path)
        via1_array(cell, via2_layer, x0_0+(300-190)/2, y0+tran_w+path_width+220+2*h_path_sep_width+h_path_width+(300-190)/2, 66, 1, 220, 190)
        via1_array(cell, via1_layer, x0_0+(300-190)/2, y0+50, 38, 1, 220, 190)
...
def source_tranarray_interconn(x0, y0, path_length, tran_num_row, i):
    ...
    box_m4 = pya.Box(x0-v_path_sep_width, y0+h_path_sep_width, x0-v_path_width-v_path_sep_width, y0+h_path_sep_width+h_path_width) 
    ...
    via1_array(cell, via3_layer, x0-v_path_sep_width-v_path_width+(300-190)/2, y0+h_path_sep_width+(300-190)/2, 57, 21, 290, 190)
    via1_array(cell, via4_layer, x0-v_path_sep_width-v_path_width+(300-190)/2, y0+h_path_sep_width+(300-190)/2, 57, 21, 290, 190)
    via1_array(cell, topvia1_layer, x0-v_path_sep_width-v_path_width+100, y0+h_path_sep_width+100, 33, 12, 420, 420)
    #via1(cell, via1_layer, x0+path_length+3*difusion_hsp+(300-190)/2-300, y0+(300-190)/2)
    
    if i<tran_num_row:
        box_2 = pya.Box(x0-v_path_sep_width+300, y0-tran_w-path_width-220-2950, x0-v_path_sep_width-v_path_width-320, y0+300+tran_vsp)
...
#    cell.shapes(layer).insert(box_dlt)
    ...
    box_m4 = pya.Box(x0+path_length+v_path_sep_width, y0+2*h_path_sep_width+h_path_width, x0+path_length+v_path_sep_width+v_path_width, y0+2*h_path_sep_width+2*h_path_width)
    cell.shapes(m4_layer).insert(box_m4)
    cell.shapes(m5_layer).insert(box_m4)
    #    via1(cell, via1_layer, x0-7*difusion_hsp+(300-190)/2, y0+(300-190)/2)
    via1_array(cell, via3_layer, x0+path_length+h_path_sep_width+300+(300-190)/2-300, y0+2*v_path_sep_width+h_path_width+(300-190)/2, 57, 21, 290, 190)
    via1_array(cell, via4_layer, x0+path_length+h_path_sep_width+300+(300-190)/2-300, y0+2*v_path_sep_width+h_path_width+(300-190)/2, 57, 21, 290, 190)
    via1_array(cell, topvia1_layer, x0+path_length+h_path_sep_width+100, y0+2*v_path_sep_width+h_path_width+100, 33, 12, 420, 420)
    if i<tran_num_row:
#        box_d = pya.Box(x0-7*difusion_hsp, y0, x0-7*difusion_hsp+300, y0+tran_vsp+300)
#        cell.shapes(m2_layer).insert(box_d)
        box_drv = pya.Box(x0+path_length+v_path_sep_width-320, y0, x0+path_length+v_path_sep_width+v_path_width+320, y0 + tran_vsp+ 300)
...
def poly_interconn(cell, layout, x0, y0):
    ...
    cell.shapes(layer).insert(box)
    via1_array(cell, via1_layer, x0+340-2500+50, y0+tran_w+180+50, 2, 2, 220, 190)
    if i<tran_num_row-1:
        box_con = pya.Box(x0+340-2500+700, y0+tran_w+180, x0+340-2500, y0+tran_w+180+tran_vsp+700)
...
def guard_ring(cell, layout, x0, y0, width, hight, path_width):
	...
	box_m1 = pya.Box(x0+width,y0, x0-v_path_sep_width-v_path_width, y0+path_width)
	box_m2 = pya.Box(x0-v_path_sep_width,y0, x0-v_path_sep_width-v_path_width, y0+path_width)
	box_nwell = pya.Box(x0-310, y0-400, x0+width+310, y0+path_width+360)
	contact_array(cell, layout, x0+200, y0+400, 4, 324)
	via1_array(cell, via1_layer, x0-v_path_sep_width-v_path_width+250, y0+250, 4, 20, 290, 190)
	via1_array(cell, via2_layer, x0-v_path_sep_width-v_path_width+250, y0+250, 4, 20, 290, 190)
	via1_array(cell, via3_layer, x0-v_path_sep_width-v_path_width+250, y0+250, 4, 20, 290, 190)
	via1_array(cell, via4_layer, x0-v_path_sep_width-v_path_width+250, y0+250, 4, 20, 290, 190)
	via1_array(cell, topvia1_layer, x0-v_path_sep_width-v_path_width+250, y0+250, 2, 11, 420, 420)
