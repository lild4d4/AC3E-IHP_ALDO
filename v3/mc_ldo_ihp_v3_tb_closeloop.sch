...
C {devices/code_shown.sym} 300 -400 0 0 {name=NGSPICE only_toplevel=true 
value="
.param mm_ok=1
.param mc_ok=1
.param temp=27
...
"}
C {/foss/designs/ihp_design/v3/ldo_v3.sym} -250 -210 0 0 {name=x1}
...
let mc_runs = 1000
let run = 0
shell rm /foss/designs/ihp_design/v3/mc_hbt_3dB.csv
***************** LOOP *********************
dowhile run < mc_runs
reset
set run=$&run
...
print DCG >> /foss/designs/ihp_design/v3/mc_hbt_3dB.csv
let run=run+1 
end
***************** LOOP *********************
.endc
...
let mc_runs = 1000
let run = 0
shell rm /foss/designs/ihp_design/v3/mc_ldo_line_reg.csv
shell rm /foss/designs/ihp_design/v3/mc_ldo_dropout.csv
***************** LOOP *********************
dowhile run < mc_runs
reset
set run=$&run
...
print line_reg >> /foss/designs/ihp_design/v3/mc_ldo_line_reg.csv

let der = deriv(vout)
meas dc V_ldo_vdropout FIND v(vdd) WHEN der=0.02 CROSS=LAST
let dropout=V_ldo_vdropout-1.2
print dropout >> /foss/designs/ihp_design/v3/mc_ldo_dropout.csv
...
"}
C {devices/code.sym} -990 160 0 0 {name=MC_Load_Reg_Dropout_Simulation only_toplevel=true
...
let mc_runs = 1000
let run = 0
shell rm /foss/designs/ihp_design/v3/mc_ldo_load_reg.csv
***************** LOOP *********************
dowhile run < mc_runs
reset
set run=$&run
...
print load_reg/9.9 >> /foss/designs/ihp_design/v3/mc_ldo_load_reg.csv
